...
var promptManager = function (result) {
    inquirer.prompt([{
        ...
        choices: ["Add new item", "Add quantity to an existing item", "Subtract quantity to an existing item"]
    }]).then(function (val) {
        ...
        if (val.choice == "Subtract quantity to an existing item") {
            subQuantity(result);
        }
    })
}
...
function subQuantity(res) {
    inquirer.prompt([{
        ...
        type: "input",
        name: "subtracted",
        message: "How much stock would you like to substract?"
    }]).then(function (val) {
        for (i = 0; i < res.length; i++) {
            if (res[i].product_name == val.product_name) {
                connection.query('UPDATE products SET stock_quantity = stock_quantity-' + val.subtracted + ' WHERE item_id=' + res[i].item_id + ';', function (err, res) {
                    if (err) throw err;
                    if (res.affectedRows == 0) {
                        console.log("That item does not exist at this time. Try selecting a different item.");
                    }
                    // else if (val.subtracted > res.stock_quantity) {
                    //     console.log("Unable to subtract more than the previous stock.");
                    //     subQuantity();
                    // } 
                    else {
                        console.log("Items have been subtracted from your inventory.");
                        makeTable();
                    }
                })
            }
